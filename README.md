...
- **Mounted filesystem monitoring**: Fanotify now marks each local disk filesystem (skipping network, overlay, squashfs, and non-disk FUSE), including separate Steam game drives, instead of monitoring only the root filesystem. Desktop mounts under `/run/media/` and `/run/mnt/` are admitted by default.
